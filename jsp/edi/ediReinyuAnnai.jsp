...
<title><bean:message key="com.title.ediReinyuAnnai"/></title>
...
<%-- 共通ヘッダー終了 --%>
<%-- 検索条件入力フォーム開始 --%>
<kit:form action="/ediReinyuAnnai_Search" styleClass="searchForm">
	<%-- タイトル開始 --%>
	<div class="title">
		<bean:message key="com.title.ediReinyuAnnai"/>
	</div>
	<%-- タイトル終了 --%>
	<%-- 検索条件 --%>
...
<%-- 登録・更新・削除フォーム開始 --%>
<kit:form action="/ediReinyuAnnai_Edit.do">
<kit:hidden name="ediReinyuAnnai_Edit" property="editable" />
<kit:hidden name="ediReinyuAnnai_Edit" property="reqType" />
<kit:hidden name="ediReinyuAnnai_Edit" property="mode" />
<kit:hidden name="ediReinyuAnnai_Edit" property="preMode" />
<kit:hidden name="ediReinyuAnnai_Edit" property="maxSu" />
			...
			<%-- 作成対象 --%>
				...
				<%-- 作成対象 --%>
					<kit:select
						...
						<kit:options collection="ediReinyuAnnai_EDI_REINYU_MAKE" optional="true" useDefault="false"/>
					</kit:select>
				</td>
				<td align=left class="botanhaba">
					<kit:button
						...
						property="btnCommit"
						tabindex="4"
						onclick="if(sendCheck('データ作成')){if(eventPreSubmit()){setReqType('ReinyuAnnaiMake');offUnloadConfirm();submit()}}">
			...
			<%-- 送信対象 --%>
				...
				<%-- 送信先区分 --%>
					<kit:select
						...
						<kit:options collection="ediReinyuAnnai_EDI_REINYU_PARTNER" optional="true" useDefault="false"/>
					</kit:select>
				</td>
				<td align=left class="botanhaba">
					<kit:button
						...
						property="btnCommit"
						tabindex="6"
						onclick="if(sendCheck('送信')){if(eventPreSubmit()){setReqType('ReinyuAnnaiEDI');offUnloadConfirm();submit()}}">
