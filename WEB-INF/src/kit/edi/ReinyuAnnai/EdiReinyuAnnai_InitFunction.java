 ...
 *
 */
public class EdiReinyuAnnai_InitFunction extends KitFunction {

	/** シリアルID */
	private static final long serialVersionUID = -1776046706302465586L;

	// -----------------------------------------
	// create log4j instance & Config LogLevel
	// -----------------------------------------
	private static String className__ = EdiReinyuAnnai_InitFunction.class.getName();
	 ...
	 */
	public EdiReinyuAnnai_InitFunction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
	...
	public String execute_() throws KitException {
		...
		// ★★送信先区分リストを作成する
		searchServ.search(KBN_EDI_REINYU_PARTNER);
		if (searchServ.getPbsRecords() != null) {
				setSession(KEY_SS_KBN_EDI_REINYU_PARTNER , new KitSelectList(searchServ.getPbsRecords()));
		}
		...
		// ★★作成対象リストを作成する
		edisearchServ.search(EDI_SEND_DATA_KB_ANY_ANNAI, EDI_DATA_CLASS_KB_REINYU, "");
		if (edisearchServ.getPbsRecords() != null) {
				setSession(KEY_SS_KBN_EDI_REINYU_MAKE , new KitSelectList(edisearchServ.getPbsRecords()));
		}


		EdiReinyuAnnai_SearchForm searchForm = new EdiReinyuAnnai_SearchForm();

		// 検索フォームをセッションに格納する
		setSession(KEY_SS_SEARCHFORM, searchForm);

		// 初期画面表示モードのセット
		EdiReinyuAnnai_EditForm editForm = new EdiReinyuAnnai_EditForm();
