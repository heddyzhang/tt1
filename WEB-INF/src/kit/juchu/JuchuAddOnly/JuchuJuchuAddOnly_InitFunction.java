 ...
 *
 */
public class JuchuJuchuAddOnly_InitFunction extends KitFunction {

	/** シリアルID */
	private static final long serialVersionUID = 7004619838480356344L;

	// -----------------------------------------
	// create log4j instance & Config LogLevel
	// -----------------------------------------
	private static String className__ = JuchuJuchuAddOnly_InitFunction.class.getName();
	 ...
	 */
	public JuchuJuchuAddOnly_InitFunction(ActionMapping mapping, ActionForm form,
	...
	public String execute_() throws KitException {
		...
		// 空のeditFormをセットする
		setRequest(KEY_SS_EDITFORM, new JuchuJuchuAddOnly_EditForm());
		...
		if (searchServ.getPbsRecords() != null) {
			KitSelectList tmpList = new KitSelectList(searchServ.getPbsRecords());
			setSession(KEY_SS_SIKIBETU, tmpList);
		}

		// 識別（データ種別CDの一部）リストを作成する
		searchServ.search(KBN_JDATAKIND);
		if (searchServ.getPbsRecords() != null) {
			// 売伝発行済以外
			KitSelectList tmpList = new KitSelectList(searchServ.getPbsRecords()).getFiltered(
					JDATAKIND_KB_TUJO, JDATAKIND_KB_TANKA_TIGAI, JDATAKIND_KB_JYUCYU_CANCEL, JDATAKIND_KB_URIDEN_CANCEL);
			setSession(KEY_SS_SIKIBETU_LIMIT, tmpList);
		}
		...
		// 初期値設定
		JuchuJuchuAddOnly_SearchForm searchForm = new JuchuJuchuAddOnly_SearchForm();
