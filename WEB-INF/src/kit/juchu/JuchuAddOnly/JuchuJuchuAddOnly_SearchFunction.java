 ...
 */
public class JuchuJuchuAddOnly_SearchFunction extends KitFunction {

	/** シリアルID */
	private static final long serialVersionUID = 4557345044610454677L;

	// -----------------------------------------
	// create log4j instance & Config LogLevel
	// -----------------------------------------
	private static String className__ = JuchuJuchuAddOnly_SearchFunction.class.getName();

	/** 明細部 */
	private JuchuJuchuAddOnly_EditForm editForm;
	 ...
	 */
	public JuchuJuchuAddOnly_SearchFunction(ActionMapping mapping, ActionForm form,
	...
	protected String execute_() throws NoDataException, KitException {
		...
		JuchuJuchuAddOnly_SearchForm searchForm = (JuchuJuchuAddOnly_SearchForm) form_;
		...
		// ----------------------------------------------------
			...
			// 確認画面を表示せずに確定処理を行う
			JuchuJuchuAddOnly_EditForm editForm_ = (JuchuJuchuAddOnly_EditForm) getSession(KEY_SS_EDITFORM);
			...
			JuchuJuchuAddOnly_EditFunction callFunc = new JuchuJuchuAddOnly_EditFunction(mapping_, editForm_, request_, response_);
	 ...
	 * 「呼出」処理を行う
	 *
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 * @throws NoDataException
	 */
	private void search(JuchuJuchuAddOnly_SearchForm searchForm) throws NoDataException {
		...
		try {
			...
			// 検索結果をリストクラスに変換
			JuchuJuchuAddOnlyList searchedList = new JuchuJuchuAddOnlyList(records);
	 ...
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 */
	private void sort(JuchuJuchuAddOnly_SearchForm searchForm) {
		...
		// 指定項目でソート
		JuchuJuchuAddOnlyList sortedList = searchedList;
		...
		if (PbsUtil.isIncluded(searchForm.getSortItem(), "SYUKA_SURYO_BOX", "SYUKA_SURYO_SET", "JYURYO_TOT")) {
			if (SORT_ASC.equals(searchForm.getSortOrder())) { // 昇順
				sortedList = (JuchuJuchuAddOnlyList) ComKitUtil.sortPbsRecordListDecimalAsc(searchedList, searchForm.getSortItem());
			} else if (SORT_DESC.equals(searchForm.getSortOrder())) { // 降順
				sortedList = (JuchuJuchuAddOnlyList) ComKitUtil.sortPbsRecordListDecimalDesc(searchedList, searchForm.getSortItem());
			}
		// 文字列としてソート
		} else {
			if (SORT_ASC.equals(searchForm.getSortOrder())) { // 昇順
				sortedList = (JuchuJuchuAddOnlyList) ComKitUtil.sortPbsRecordListAsc(searchedList, searchForm.getSortItem());
			} else if (SORT_DESC.equals(searchForm.getSortOrder())) { // 降順
				sortedList = (JuchuJuchuAddOnlyList) ComKitUtil.sortPbsRecordListDesc(searchedList, searchForm.getSortItem());
			}
		}
	 ...
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 */
	private void choice(JuchuJuchuAddOnly_SearchForm searchForm) {
	 ...
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 */
	private void add(JuchuJuchuAddOnly_SearchForm searchForm) {
		...
		// =================================================================
		// ヘッダー部
		JuchuJuchuAddOnlyList editList = new JuchuJuchuAddOnlyList(INT_EDIT_ONE_RECORD);
		editList.setRecordsParPage(searchForm.getMaxSu());
		editList.setAddMode();
		// ディテール部
		JuchuJuchuAddOnlyDtList editDtList = new JuchuJuchuAddOnlyDtList(searchForm.getMaxSuDt());
	 ...
	 *
	 * @param editRec JuchuJuchuAddOnlyRecord
	 * @param tatesenRec MastrTatesenOrositenRecord
	 */
	private void setDefaultForAdd(JuchuJuchuAddOnlyRecord editRec, MastrTatesenOrositenRecord tatesenRec) {
		// 利用区分
	 ...
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 */
	private void manipulateEdit(JuchuJuchuAddOnly_SearchForm searchForm) throws KitException {
		...
		// フォームを初期化
		this.editForm = new JuchuJuchuAddOnly_EditForm();

		// モードを取得
		String nextMode = ComKitUtil.getModeByReqTypeOnSearchForm(searchForm.getReqType());

		// EditListを取得する
		JuchuJuchuAddOnlyList editList = (JuchuJuchuAddOnlyList) getSession(KEY_SS_EDITLIST); // ヘッダー部
		JuchuJuchuAddOnlyDtList editDtList = (JuchuJuchuAddOnlyDtList) getSession(KEY_SS_EDITLIST_DT); // ディテール部
		JuchuJuchuAddOnlyList searchedList = (JuchuJuchuAddOnlyList) getSession(KEY_SS_SEARCHEDLIST);
		...
		// 初期リストを準備する
		// ヘッダー部
		JuchuJuchuAddOnlyList initList = (JuchuJuchuAddOnlyList) editList.copy();
		setSession(KEY_SS_INITLIST, initList);
		// ディテール部
		JuchuJuchuAddOnlyDtList initDtList = (JuchuJuchuAddOnlyDtList) editDtList.copy();
		setSession(KEY_SS_INITLIST_DT, initDtList);
		// カテゴリデータ
		JuchuJuchuAddOnly_SearchService searchServ = new JuchuJuchuAddOnly_SearchService(
				getComUserSession(), getDatabase(), isTransaction(), getActionErrors());
		JuchuJuchuAddOnlyCatList initCatList = searchServ.getListJuchuJuchuAddOnlyCat(editRec.getJyucyuNo());
		setSession(KEY_SS_INITLIST_CAT, initCatList);

		// 整形済み表示用受注NO
		editRec.setJyucyuNoFormattedView(checkServ.formatJyucyuNo(editRec.getJyucyuNo())); // 受注NOをフォーマット
	 ...
	 * @param searchForm JuchuJuchuAddOnly_SearchForm
	 */
	private void manipulateView(JuchuJuchuAddOnly_SearchForm searchForm) {
		...
		JuchuJuchuAddOnly_SearchService searchServ = new JuchuJuchuAddOnly_SearchService(
				getComUserSession(), getDatabase(), isTransaction(), getActionErrors());
		JuchuJuchuAddOnlyCatList initCatList = searchServ.getListJuchuJuchuAddOnlyCat(editForm.getJyucyuNo());
		setSession(KEY_SS_INITLIST_CAT, initCatList);
	 ...
	 * @param editDtList
	 */
	private void setTabindexes(String reqType, JuchuJuchuAddOnlyList editList, JuchuJuchuAddOnlyDtList editDtList) {
