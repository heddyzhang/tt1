 ...
 */
public class JuchuJuchuAddOnly_SearchService extends KitService {

	/** シリアルID */
	private static final long serialVersionUID = -5208350006992356666L;

	// -----------------------------------------
	// create log4j instance & Config LogLevel
	// -----------------------------------------
	private static String className__ = JuchuJuchuAddOnly_SearchService.class.getName();
	 ...
	 */
	public JuchuJuchuAddOnly_SearchService(ComUserSession cus, PbsDatabase db_, boolean isTran,
			ActionErrors ae) {
	...
	/**
	 * 受付専用受注追加入力（受注データ／ヘッダー部）検索のSQL生成と実行.
	 *
	 * @param searchForm 検索フォーム
	 * @return 検索結果
	 * @throws DataNotFoundException
	 */
	public PbsRecord[] execute(JuchuJuchuAddOnly_SearchForm searchForm)
			throws DataNotFoundException {

		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）検索処理 開始");

		List<String> bindList = new ArrayList<String>();

		category__.info("受付専用受注追加入力（受注データ／ヘッダー部） >> 検索処理 >> SQL文生成");
		String searchSql = getSqlSearch(searchForm, bindList);

		category__.info("受付専用受注追加入力（受注データ／ヘッダー部） >> 検索処理 >> prepare");
		...
		try {
			...
			category__.info("受付専用受注追加入力（受注データ／ヘッダー部） >> execute");
			db_.execute(iCc);
		...
		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）検索処理 終了");

		return records_;
	}

	/**
	 * 受付専用受注追加入力（受注データ／ヘッダー部）の検索SQLの作成
	 *
	 * @param searchForm
	 * @param bindList
	 * @return sSql
	 */
	private String getSqlSearch(JuchuJuchuAddOnly_SearchForm searchForm, List<String> bindList) {
		...
		// 識別が"8：受注キャンセル"または"9：売伝キャンセル"
		// または　受注NO枝番が"0"以外　が対象・・・配送受付窓口用
		// かつ　識別が"7：売伝発行済"を除く
		sSql.append("AND (JCUHD.SYUBETU_CD IN (?, ?) \n");
		sSql.append("OR JCUHD.JYUCYU_NO NOT LIKE ? ) \n");
		sSql.append("AND JCUHD.SYUBETU_CD <> ? \n");
		bindList.add(JDATAKIND_KB_JYUCYU_CANCEL);
		bindList.add(JDATAKIND_KB_URIDEN_CANCEL);
		bindList.add(ComPopupUtil.getWildCardLeft("0"));
		bindList.add(JDATAKIND_KB_DENPATU_SUMI);

		// 受注NOの前から2文字目が'Z'は対象外・・・蔵直データを除く
		sSql.append("AND SUBSTR(JCUHD.JYUCYU_NO, 2, 1) <> 'Z' \n");
	...
	/**
	 * 受付専用受注追加入力（受注データ／ヘッダー部）リストを取得する
	 *
	 * @param jyucyuNo 受注NO
	 * @return JuchuJuchuAddOnlyList 検索結果
	 */
	public JuchuJuchuAddOnlyList getListJuchuJuchuAddOnly(String jyucyuNo) {
		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）リスト検索処理 開始");

		List<String> bindList = new ArrayList<String>();

		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）リスト >> 検索処理 >> SQL文生成");
		String searchSql = getJuchuAddOnlyListSearchSql(jyucyuNo, bindList);

		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）リスト >> 検索処理 >> prepare");
		Integer iCc = db_.prepare(true,searchSql);
		...
		try {
			...
			category__.info("受付専用受注追加入力（受注データ／ヘッダー部）リスト >> execute");
			db_.execute(iCc);
			records_ = db_.getPbsRecords(iCc);

		} catch (Exception e) {
		}
		category__.info("受付専用受注追加入力（受注データ／ヘッダー部）リスト検索処理 終了");

		// 検索結果を変換
		JuchuJuchuAddOnlyList searchedList = new JuchuJuchuAddOnlyList();
		if (records_ != null) {
			searchedList = new JuchuJuchuAddOnlyList(records_);
		}
	...
	/**
	 * 受注NOから受付専用受注追加入力（受注データ／ヘッダー部）リストを取得するSQL
	 *
	 * @param jyucyuNo
	 * @param bindList
	 * @return sSql
	 */
	private String getJuchuAddOnlyListSearchSql(String jyucyuNo, List<String> bindList) {
	...
	/**
	 * 受付専用受注追加入力（受注データ／ディテール部）リストを取得する
	 *
	 * @param jyucyuNo 受注NO
	 * @return JuchuJuchuAddOnlyDtList 検索結果
	 */
	public JuchuJuchuAddOnlyDtList getListJuchuJuchuAddOnlyDt(String jyucyuNo) {
		category__.info("受付専用受注追加入力（受注データ／ディテール部）リスト検索処理 開始");

		List<String> bindList = new ArrayList<String>();

		category__.info("受付専用受注追加入力（受注データ／ディテール部）リスト >> 検索処理 >> SQL文生成");
		String searchSql = getJuchuAddOnlyDtListSearchSql(jyucyuNo, bindList);

		category__.info("受付専用受注追加入力（受注データ／ディテール部）リスト >> 検索処理 >> prepare");
		Integer iCc = db_.prepare(true,searchSql);
		...
		try {
			...
			category__.info("受付専用受注追加入力（受注データ／ディテール部）リスト >> execute");
			db_.execute(iCc);
			records_ = db_.getPbsRecords(iCc);

		} catch (Exception e) {
		}
		category__.info("受付専用受注追加入力（受注データ／ディテール部）リスト検索処理 終了");

		// 検索結果を変換
		JuchuJuchuAddOnlyDtList searchedList = new JuchuJuchuAddOnlyDtList();
		if (records_ != null) {
			searchedList = new JuchuJuchuAddOnlyDtList(records_);
		}
	...
	/**
	 * 受注NOから受付専用受注追加入力（受注データ／ディテール部）リストを取得するSQL
	 *
	 * @param jyucyuNo
	 * @param bindList
	 * @return sSql
	 */
	private String getJuchuAddOnlyDtListSearchSql(String jyucyuNo, List<String> bindList) {
	...
	/**
	 * 受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リストを取得する
	 *
	 * @param jyucyuNo 受注NO
	 * @return JuchuJuchuAddOnlyDtList 検索結果
	 */
	public JuchuJuchuAddOnlyCatList getListJuchuJuchuAddOnlyCat(String jyucyuNo) {
		category__.info("受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リスト検索処理 開始");

		List<String> bindList = new ArrayList<String>();

		category__.info("受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リスト >> 検索処理 >> SQL文生成");
		String searchSql = getJuchuAddOnlyCatListSearchSql(jyucyuNo, bindList);

		category__.info("受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リスト >> 検索処理 >> prepare");
		Integer iCc = db_.prepare(true,searchSql);
		...
		try {
			...
			category__.info("受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リスト >> execute");
			db_.execute(iCc);
			records_ = db_.getPbsRecords(iCc);

		} catch (Exception e) {
		}
		category__.info("受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リスト検索処理 終了");

		// 検索結果を変換
		JuchuJuchuAddOnlyCatList searchedList = new JuchuJuchuAddOnlyCatList();
		if (records_ != null) {
			searchedList = new JuchuJuchuAddOnlyCatList(records_);
		}
	...
	/**
	 * 受注NOから受付専用受注追加入力（予定出荷先別商品カテゴリデータ）リストを取得するSQL
	 *
	 * @param jyucyuNo
	 * @param bindList
	 * @return sSql
	 */
	private String getJuchuAddOnlyCatListSearchSql(String jyucyuNo, List<String> bindList) {
