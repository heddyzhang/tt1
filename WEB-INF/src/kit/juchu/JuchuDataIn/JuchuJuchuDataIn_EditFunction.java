 ...
 */
public class JuchuJuchuDataIn_EditFunction extends KitFunction {

	/** シリアルID */
	private static final long serialVersionUID = -3884543943754836555L;

	/** クラス名. */
	private static String className__ = JuchuJuchuDataIn_EditFunction.class.getName();
	...
	/** 編集フォーム */
	private JuchuJuchuDataIn_EditForm editForm = null;
	/** 編集リスト */
	private JuchuJuchuDataInList editList = null; // ヘッダー部
	private JuchuJuchuDataInDtList editDtList = null; // ディテール部
	 ...
	 */
	public JuchuJuchuDataIn_EditFunction(ActionMapping mapping, ActionForm form,
	...
	public String execute_() throws KitException {
		...
		// オブジェクトを取得
		editForm = (JuchuJuchuDataIn_EditForm) form_;
	...
	private void back() {
		...
		// 編集リストを取得する
		editList = (JuchuJuchuDataInList) getSession(KEY_SS_EDITLIST); // ヘッダー部
		editDtList = (JuchuJuchuDataInDtList) getSession(KEY_SS_EDITLIST_DT); // ディテール部
		// maxSuまで空レコードで埋める
		...
		/* 抹消(確認) → 表示 */
			...
			// 初期リストを呼出し
			JuchuJuchuDataInList initList = (JuchuJuchuDataInList) getSession(KEY_SS_INITLIST); // ヘッダー部
			JuchuJuchuDataInDtList initDtList = (JuchuJuchuDataInDtList) getSession(KEY_SS_INITLIST_DT); // ディテール部

			// 編集リストを再生する
			editList = (JuchuJuchuDataInList) initList.copy(); // ヘッダー部
			editDtList = (JuchuJuchuDataInDtList) initDtList.copy(); // ディテール部
	...
	private void commitAdd() {
		...
		// ・例外catch処理記述
		// ========================================================================
		JuchuJuchuDataIn_UpdateService updateServ = new JuchuJuchuDataIn_UpdateService(
	...
	private void commitDelete() {
		...
		// 初期リストを取得する
		JuchuJuchuDataInCatList initCatList = (JuchuJuchuDataInCatList) getSession(KEY_SS_INITLIST_CAT); // カテゴリデータ
	...
	private void confirmAdd() throws KitException {
		...
		// ヘッダー部
		// =============================================
		editList.setPageList(editForm.getJuchuJuchuDataInList());
	...
	private void select() throws KitException {
		...
		JuchuJuchuDataIn_SearchForm searchForm = (JuchuJuchuDataIn_SearchForm) getSession(KEY_SS_SEARCHFORM);
		...
		// 選択結果を取得
		// ヘッダー部
		JuchuJuchuDataInRecord selectedRec = checkServ.getRecSelected();
		JuchuJuchuDataInList editList = new JuchuJuchuDataInList(selectedRec);

		// 検索実行
		// ディテール部
		JuchuJuchuDataIn_SearchService searchServ = new JuchuJuchuDataIn_SearchService(
				getComUserSession(), getDatabase(), isTransaction(), getActionErrors());
		JuchuJuchuDataInDtList editDtList = searchServ.getListJuchuJuchuDataInDt(editForm.getJyucyuNo());
		// maxSuまで空レコードで埋める
		editDtList.initListToMax(editDtList.size(), editForm.getMaxSuDt());
		...
		// 蔵直データの場合、フラグをセットする
		if (PbsUtil.isEqual((editRec.getJyucyuNo()).substring(1, 2), KURAC_CHAR)) {
			searchForm.setKuracFlg(KURAC_CHAR); // 蔵直データ
		} else {
			searchForm.setKuracFlg(CHAR_BLANK); // 蔵直データ以外
		}
	 ...
	 * @param editDtList
	 */
	private void setTabindexConfirmAndBack(String reqType, JuchuJuchuDataInList editList, JuchuJuchuDataInDtList editDtList) {
	...
	private void trimSpace() {
		JuchuJuchuDataInRecord hdRec = (JuchuJuchuDataInRecord) (editForm.getJuchuJuchuDataInList()).getFirstRecord();
		// 前後のスペースを除去
	 ...
	 * @throws KitException
	 */
	private void reCreateList(JuchuJuchuDataIn_CheckService checkServ, JuchuJuchuDataInRecord hdRec) throws KitException {
		ComUserSession cus = getComUserSession();
		...
		editDtList.setPageList(editForm.getJuchuJuchuDataInDtList());
		...
		for (PbsRecord tmp : editDtList) {
			JuchuJuchuDataInDtRecord dtRec = (JuchuJuchuDataInDtRecord) tmp;
